...
import { createRoom, joinRoom, applyCommand, projectView, HINTS, SHAPES, SYMBOLS, GLYPHS, makeId } from "./core/game.js";
...
function renderHome() {
  ...
  put('resume-link', lastRoom ? `<button class="text-button tiny" data-action="resume">Return to relay ${esc(lastRoom)} →</button>` : '');
}
...
function renderGame() {
  ...
  put('game-heading',`<p class="kicker">CHAMBER 0${view.chamber} OF 03</p><h1>${esc(view.title)}</h1>`);
  ...
  put('stage-bar',`<span class="era-tag ${view.role}">${view.role==='past'?'☀ THE PAST':'☾ THE FUTURE'} · ${esc(view.players[view.role]?.name)}</span><span class="year">${view.role==='past'?'1874 · Before the silence':'2074 · After the fall'}</span>`);
  put('scene',scene(view));
  put('event-line',esc(view.event));
  put('chapter-progress',['The garden','The workshop','The observatory'].map((title,i)=>`<div class="${view.chamber===i+1?'current':view.chamber>i+1?'complete':''}">${view.chamber>i+1?'✓':`0${i+1}`} ${title}</div>`).join(''));
  put('controls',controlMarkup());
  ...
  const signalNames=['Hold on','Anchor now','Change it','Ready',...(view.chamber===2?SHAPES.map(cap):view.chamber===3?SYMBOLS.map(cap):[])];
...
function renderEnding() {
  mount('ending', `<main id="main" class="ending"><div id="ending-mode"></div><div class="ending-art">${scene(null,'ending')}</div><p class="kicker">The relay is complete</p><h1>You didn't escape time.<br>You brought each other home.</h1><p>The garden remembers. The conservatory is alive again.<br>And for the first time, you are standing in the same tomorrow.</p><div id="ending-core"></div></main>`);
...
async function enterFromForm() {
  ...
  }catch(error){busy=false;notify(error.message);render();}
}
...
function startPractice() {
  ...
  practice=createRoom('SOLO','Your past self','practice-past','not-a-live-credential',seed);
...
function home() {
  ...
  connection.stop();practice=null;view=null;status='offline';busy=false;mounted='';chatDraft='';tab='create';replaceUrl('/');render();
}
function showGuide() {
  let dialog=document.getElementById('guide-dialog');
  if(!dialog){
    dialog=document.createElement('dialog');dialog.id='guide-dialog';
    dialog.innerHTML=`<p class="kicker">A little guidance through time</p><h2>Neither of you can get home alone.</h2><p>Play in two different browsers or on two devices. The past changes the environment. The future sees what those choices become, and can preserve one object with a time anchor.</p><div class="how-steps"><div><div class="step-number">01 · TALK</div><h3>Share what you see.</h3><p>Your screens hold different information. Use the private messages and quick signals, or talk on your own voice call.</p></div><div><div class="step-number">02 · HOLD</div><h3>Anchor the right moment.</h3><p>An anchor freezes an object's current condition. It cannot turn a broken object into a working one. Release it to try again.</p></div><div><div class="step-number">03 · CHANGE</div><h3>Make room for tomorrow.</h3><p>Once something important is preserved, change the past again. Find the way forward together. Each secured chamber returns your anchor.</p></div></div><p class="action-note">No countdown and no irreversible failures. Hints are optional. Restarting a chamber takes both players' agreement. In solo practice, use “Switch eras” to operate both sides yourself.</p><button class="primary" data-action="close-guide">Let's find each other →</button>`;
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(action==='copy-code'||action==='copy-link'){
      ...
      try{await navigator.clipboard.writeText(text);notify(action==='copy-code'?'Relay code copied.':'Invitation copied. Send it to your playing partner.');}catch{notify(`Share this relay code: ${view.code}`);}return;
    }
    if(!view)return;
    if(action==='ready'){send({type:'ready',value:!view.players[view.role].ready});return;}
    if(['water','gear','power','beam'].includes(action)){send({type:action,value});return;}
    if(action==='ring'){send({type:'ring',index:Number(button.dataset.index),value});return;}
    if(['anchor','release','cross','ride','pulse','decline'].includes(action)){send({type:action});return;}
...
render();
if(roomInput&&/^[A-HJ-NP-Z2-9]{6}$/.test(roomInput))enter(roomInput);
